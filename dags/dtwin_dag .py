"""
Digital Twin DAG Module.

This module defines the Airflow DAG for the digital twin workflow.
It orchestrates model fitting, prediction, and state estimation tasks
for bioreactor experiments.
"""
...
try:
    ...
    print(
        "Host path has not been addded to the airflow UI variables or it has not been done correctly!"
    )
...
with DAG(
    ...
    start_dtwin = base_docker_node(
        ...
        command=[
            "python",
            "-c",
            "from Node_start_dtwin import start_dtwin; start_dtwin()",
        ],
    )
            ...
            for minutes in iter_minutes:
                ...
                run_dtwin = base_docker_node(
                    ...
                    command=[
                        "python",
                        "-c",
                        "from Node_run_dtwin import run_dtwin; run_dtwin()",
                    ],
                )
    ...
    for hours in range(int(t_duration)):
        ...
        with TaskGroup(
            group_id=f"{hours + 2}_hour{'s' if hours+1 > 1 else ''}_update_tasks"
        ):
            ...
            for minutes in iter_minutes_dtwin_update:
                ...
                update_dtwin = base_docker_node(
                    ...
                    command=[
                        "python",
                        "-c",
                        "from methodParamEst import update_group; update_group(['19419','19420','19427','19428','19435','19436'])",
                    ],
                )
