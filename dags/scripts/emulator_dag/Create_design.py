...
for i in range(mbr_list.shape[0]):
    Params_ref_row = (
        Params_ref * (1 + 0.33 * (np.random.random(len(Params_ref)) - 0.5) / 2)
    ).tolist()
    Params[i] = (
        Params_ref_row
        + (850 * (1 + 0.33 * (np.random.random(1) - 0.5) / 2)).tolist()
...
for i1 in Exp_list:
    ...
    EMULATOR_config[i1]["Pulse_profile"] = {
        ...
        "Feed_pulse": (5 + np.zeros(len(time_pulses.tolist()))).tolist(),
        "time_dextrine": time_dextrine.tolist(),
        "Feed_dextrine": (0 + np.zeros(len(time_dextrine.tolist()))).tolist(),
        "time_enzyme": time_enzyme.tolist(),
        "Feed_enzyme": (0 + np.zeros(len(time_enzyme.tolist()))).tolist(),
    }
    ...
    EMULATOR_config[i1]["time_sample"]["DOT"] = np.linspace(
