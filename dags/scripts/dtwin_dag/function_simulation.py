...
def function_simulation(ts0,Xo0,u0,THs,D0={}):
    ...
    for i in t_u[:-1]:
        ...
        if i in t_u_pulse:
            ...
            Xo1[1]=Xo1[1]+uu_pulse[index_u_pulse]*1e-6*u0[0]/0.01 # Add substrate
        ...
        if i in t_u_dextrine:
            ...
            Xo1[6]=Xo1[6]+uu_dextrine[index_u_dextrine]*1e-6*u0[5]/0.01*(.46) # Add Gs
            Xo1[7]=Xo1[7]+uu_dextrine[index_u_dextrine]*1e-6*u0[5]/0.01*(1-.46) # Add Gr
            Xo1[5]=Xo1[5]+uu_dextrine[index_u_dextrine]*1e-6 # Add volume

        if i in t_u_enzyme:
            index_u_enzyme=int(np.where(t_u_enzyme==t_u[ni])[0][0])
            Xo1[8]=Xo1[8]+uu_enzyme[index_u_enzyme]*1e-6*u0[6]/0.01*0.7 # Add Enzyme
